...
import random
import torch
# import spaces  # Commented out for local execution
...
# @spaces.GPU(duration=300)  # Commented out for local execution
def infer(
    images,
    prompt,
    seed=42,
    ...
    with gr.Column(elem_id="col-container"):
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                with gr.Row():
                    use_output_btn = gr.Button("â†—ï¸ Use as input", variant="secondary", size="sm", visible=False)
                    turn_video_btn = gr.Button("ðŸŽ¬ Turn into Video", variant="secondary", size="sm", visible=False)
                output_video = gr.Video(label="Generated Video", autoplay=True, visible=False)

                with gr.Row(visible=False):
                    gr.Markdown("### ðŸ"œ History")
                    clear_history_button = gr.Button("ðŸ—'ï¸ Clear History", size="sm", variant="stop")
